...
const Recibidas = () => {
  ...
  const visibleModal = (visible, sms) => {
    ...
    // const tarjeta = json['tarjeta'];
    // const monto = json['monto'];
    // const transaccion = json['transaccion'];

    // setQrcode(`Tarjeta destino - ${tarjeta}
    //   Monto - ${monto}
    //   Nro. Transaccion - ${transaccion}`);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      const allRows = await db.getAllAsync(
        "SELECT sms.*, subcategoria.* FROM sms LEFT JOIN subcategoria ON sms.id_subcategoria = subcategoria.id WHERE sms.id_subcategoria = 3 GROUP BY date ORDER BY date DESC;"
      );
      setData(allRows);
      setIsLoading(false);
    }
