	...
	useEffect(() => {
		...
		async function fetchData() {
			...
			const allRows = await db.getAllAsync(
				"SELECT sms.*, subcategoria.* FROM sms LEFT JOIN subcategoria ON sms.id_subcategoria = subcategoria.id WHERE sms.id_subcategoria = 9 GROUP BY date ORDER BY date DESC;",
			);
			setData(allRows);
			setIsLoading(false);
		}
